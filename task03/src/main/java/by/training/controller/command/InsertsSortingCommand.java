 ...
 *
 */
public class InsertsSortingCommand <T extends Comparable<T>> implements Command {

	private static final Logger LOG = LogManager.getLogger(InsertsSortingCommand.class);
	...
	public String execute(String[] request) {
		...
		try {
			...
			if (ascending) {
				provider.getInsertsSorting().sortIncr(array);
			} else {
				provider.getInsertsSorting().sortDecr(array);
			}
		...
		return "0___inserts_sort_success";
	}
