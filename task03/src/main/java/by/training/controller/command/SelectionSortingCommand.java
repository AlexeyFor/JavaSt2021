 ...
 *
 */
public class SelectionSortingCommand <T extends Comparable<T>> implements Command {

	private static final Logger LOG = LogManager.getLogger(SelectionSortingCommand.class);
	...
	public String execute(String[] request) {
		...
		try {
			...
			if (ascending) {
				provider.getSelectionSorting().sortIncr(array);
			} else {
				provider.getSelectionSorting().sortDecr(array);
			}
		...
		return "0___selection_sort_success";
	}
