 ...
 *
 */
public class ShakeSortingCommand <T extends Comparable<T>> implements Command {

	private static final Logger LOG = LogManager.getLogger(ShakeSortingCommand.class);
	...
	public String execute(String[] request) {
		...
		try {
			...
			if (ascending) {
				provider.getShakeSorting().sortIncr(array);
			} else {
				provider.getShakeSorting().sortDecr(array);
			}
