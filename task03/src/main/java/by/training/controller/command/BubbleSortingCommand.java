 ...
 *
 */
public class BubbleSortingCommand<T extends Comparable<T>> implements Command {
	...
	public String execute(String[] request) {
		...
		try {
			...
			if (ascending) {
				provider.getBubbleSorting().sortIncr(array);
			} else {
				provider.getBubbleSorting().sortDecr(array);
			}
		...
		return "0___bubble_sort_success";
	}
